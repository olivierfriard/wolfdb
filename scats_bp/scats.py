...
def search_samples():
    if request.method == "GET":
        with fn.conn_alchemy().connect() as con:
            t0 = time.time()
            sql = text("SELECT * FROM scats_list_mat ")
    ...
    if request.method == "POST":
        with fn.conn_alchemy().connect() as con:
            ...
            sql = text(
                (
                    "SELECT * FROM scats_list_mat WHERE "
            ...
            return render_template(
                ...
                execution_time=round(time.time() - t0, 3),
                results=results,
                search_item=request.form["search"] if request.form["search"] else "View all samples",
            )
