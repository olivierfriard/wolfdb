...
def check_genetic_profile():
    ...
    if request.method == "POST":
        if "\t" in request.form["genetic_profile"].strip():
            ...
            flash(fn.alert_danger("The separator between values must be space or a TAB character"))
            return render_template(
                "check_genetic_profile.html",
                loci=" ".join([x for x in fn.get_loci_list().keys() if x != "SRY"]),
        ...
        genetic_profile = [x.replace("-", "") for x in request.form["genetic_profile"].strip().split(separator)]
        ...
        with fn.conn_alchemy().connect() as con:
            for row in (
                ...
                if row["loci_values"] is not None and row["loci_values"].split(";") == genetic_profile:
                    print(row["wa_code"])
