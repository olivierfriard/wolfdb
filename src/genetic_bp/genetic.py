...
def genotype_without_wa_loci():
    ...
    with fn.conn_alchemy().connect() as con:
        ...
        out.append(
            "<thead><tr><th>Genotype ID</th><th>Number of recaptures</th><th>WA codes / mtDna</th></tr></thead>\n"
        )
        out.append("<tbody>")
        for row in con.execute(sql).mappings().all():
            out.append("<tr>")
            out.append(
                f"<td>{row['genotype_id']}</td><td>{row['n_recaptures']}</td><td>{row['wa_code_mtDNA']}</td>"
            )
            out.append("</tr>\n")
        out.append("</tbody>")
        out.append("</table>")
...
@app.route(
    "/wa_genetic_samples2/<int:offset>/<limit>/<filter>",
    methods=(
        "GET",
        "POST",
    ),
)
@app.route("/wa_genetic_samples2/<int:offset>/<limit>/<filter>/<mode>")
@fn.check_login
def wa_genetic_samples2(offset: int, limit: int | str, filter="all", mode="web"):
    """
    display genetic data for WA code
    filter: all / with notes / red_flag / no_values
    """
    ...
    match filter:
        case "all":
            sql_all = f"SELECT COUNT(*) OVER () AS total_count, * FROM {table} WHERE (date BETWEEN :start_date AND :end_date OR date IS NULL) "

        case "with_loci_values":
            sql_all = (
                f"SELECT COUNT(*) OVER () AS total_count, * FROM {table} WHERE (date BETWEEN :start_date AND :end_date OR date IS NULL) "
                " AND wa_code IN "
                "(SELECT w.wa_code "
                ...
                "WHERE "
                "    (NULLIF(alleles #>> '{a,value}', '') IS NOT NULL AND (alleles #>> '{a,value}') NOT IN ('-', '0')) "
                "    OR "
                "    (NULLIF(alleles #>> '{b,value}', '') IS NOT NULL AND (alleles #>> '{b,value}') NOT IN ('-', '0')) "
                ")) "
            )

        case "with_notes":  # genotype notes
            sql_all = (
                f"SELECT COUNT(*) OVER () AS total_count, * FROM {table} WHERE (date BETWEEN :start_date AND :end_date OR date IS NULL) "
                "AND notes != '' AND notes IS NOT NULL "
            )

        case "with_loci_notes":  # loci notes
            sql_all = (
                f"SELECT COUNT(*) OVER () AS total_count, * FROM {table} WHERE (date BETWEEN :start_date AND :end_date OR date IS NULL) "
                " AND wa_code IN "
                "(SELECT w.wa_code "
...
@fn.check_login
def wa_genetic_samples3():
    """
    display genetic data for WA code
    filter: all / with notes / red_flag / no_values
    """

    limit = request.args.get("limit", default="unknown")
    offset = request.args.get("offset", default="unknown")
    offset = request.args.get("offset", default="unknown")

    return render_template(
        "wa_genetic_samples_list_limit2.html",
        limit=limit,
        offset=offset, filter='with_notes'
    )
...
def wa():
    ...
    only_active = "1" in request.args.getlist("only_active")
    only_verified = "1" in request.args.getlist("only_verified")

    print(only_active, only_verified)

    items = f"only_active: {only_active} only_verified {only_verified}"
...
@app.route("/check_genetic_profile", methods=["GET", "POST"])
@fn.check_login
def check_genetic_profile():
    """
    check if a genetic profile is present in database
    """
    ...
    if request.method == "POST":
        if "\t" in request.form["genetic_profile"].strip():
            separator = "\t"
        elif " " in request.form["genetic_profile"].strip():
            separator = " "
        else:
            flash(
                fn.alert_danger(
            ...
            return render_template(
                "check_genetic_profile.html",
                loci=" ".join([x for x in fn.get_loci_list().keys() if x != "SRY"]),
                default=request.form["genetic_profile"],
            )

        genetic_profile = [
            x.replace("-", "")
            for x in request.form["genetic_profile"].strip().split(separator)
        ]
        # check number of values
        if len(genetic_profile) != 42:
            flash(fn.alert_danger("The number of loci must be 42."))
            return render_template(
                "check_genetic_profile.html",
                loci=" ".join([x for x in fn.get_loci_list().keys() if x != "SRY"]),
                default=request.form["genetic_profile"],
        ...
        # check if loci values are numeric
        try:
            [int(x) for x in genetic_profile if x != ""]
        except Exception:
            flash(fn.alert_danger("Some value are not numeric"))
            return render_template(
                "check_genetic_profile.html",
                loci=" ".join([x for x in fn.get_loci_list().keys() if x != "SRY"]),
                default=request.form["genetic_profile"],
        ...
        with fn.conn_alchemy().connect() as con:
            for row in (
                con.execute(
                    text(
                        (
                            "SELECT "
                            "  wl.wa_code, "
                            "  STRING_AGG(wl.val_text::text, ';' ORDER BY ld.position, wl.allele) AS loci_values "
                            "FROM ( "
                            "  SELECT DISTINCT ON (wa_code, locus, allele) "
                            "    wa_code, "
                            "    locus, "
                            "    allele, "
                            "COALESCE(val::text, '') AS val_text, "
                            "    timestamp "
                            "  FROM "
                            "    wa_locus "
                            "  ORDER BY "
                            "    wa_code, locus, allele, timestamp DESC "
                            ") wl "
                            "JOIN "
                            "  loci ld ON wl.locus = ld.name "
                            "GROUP BY wl.wa_code; "
                            #                            "SELECT "
                            #                            "  wl.wa_code, "
                            #                            "  STRING_AGG(wl.val::text, ';' ORDER BY ld.position, wl.allele) AS loci_values "
                            #                            "FROM ( "
                            #                            "  SELECT DISTINCT ON (wa_code, locus, allele) "
                            #                            "    wa_code, "
                            #                            "    locus, "
                ...
                if (
                    row["loci_values"] is not None
                    and row["loci_values"].split(";") == genetic_profile
                ):
                    print(row["wa_code"])
        ...
        return render_template(
            "check_genetic_profile.html",
            genetic_profile=" ".join(["-" if x == "" else x for x in genetic_profile]),
            out=out,
            loci=" ".join([x for x in fn.get_loci_list().keys() if x != "SRY"]),
        )
...
def load_wa_from_spreadsheet():
    ...
    if request.method == "POST":
        ...
        r, msg, wa_results, wa_loci = wa_import.extract_wa_data_from_spreadsheet(
            filename
        )
        if r:
            flash(
            ...
            with fn.conn_alchemy().connect() as con:
                ...
                sql = text(
                    f"SELECT wa_code FROM wa_results WHERE wa_code IN ('{wa_list}')"
                )
                wa_to_update = [
                    row["wa_code"] for row in con.execute(sql).mappings().all()
                ]
    ...
    with fn.conn_alchemy().connect() as con:
        ...
        for idx in wa_loci:
            ...
            for k in data:
                if k == "wa_code":
                    continue
                locus, allele = k.split("_")
                val = data[k] if data[k] != "-" else None
                params = {
                    "wa_code": data["wa_code"],
                    "locus": locus,
                    "allele": allele,
